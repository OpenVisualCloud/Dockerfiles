...
# build gst-core
ARG GSTCORE_REPO=https://gitlab.freedesktop.org/gstreamer/gstreamer/-/archive/GSTCORE_VER/gstreamer-GSTCORE_VER.tar.gz
RUN cd BUILD_HOME && \
...
RUN cd BUILD_HOME/gstreamer-GSTCORE_VER && \
    ...
    -Dqt5=disabled \
    -Dpython=enabled \
    -Dcustom_subprojects="gst-python,gst-libav,gst-plugins-base,gst-plugins-good,gst-plugins-bad,gst-plugins-ugly" && \
    cd build && \
    ninja install && \
