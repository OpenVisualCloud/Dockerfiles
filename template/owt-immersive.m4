...
# Install 360scvp
RUN cd /home && \
    git clone ${SCVP_REPO} Immersive-Video-Sample && \
...
RUN git config --global user.email "you@example.com" && \
    ...
    cd ${SERVER_PATH}/build/libdeps/build/include && wget https://github.com/nlohmann/json/releases/download/v3.6.1/json.hpp && \
    ...
    make install DESTDIR=/home/build && \
    make install && \
    cp lib360SCVP.so /home/build/usr/local/ifelse(index(DOCKER_IMAGE,ubuntu),-1,lib64,lib/x86_64-linux-gnu) && \
    cp lib360SCVP.so /usr/local/ifelse(index(DOCKER_IMAGE,ubuntu),-1,lib64,lib/x86_64-linux-gnu) && \
