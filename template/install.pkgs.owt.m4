...
RUN ifelse(index(DOCKER_IMAGE,xeon-),-1,ln -s /opt/intel/mediasdk/lib /opt/intel/mediasdk/lib64 &&,) echo -e "\x1b[32mInstalling dependent components and libraries via apt-get...\x1b[0m" && \
    touch /home/test && \
    sed -i "/lssl/i\'\-L\/usr\/local\/ssl\/lib\'`,'" ifelse(index(DOCKER_IMAGE,-dev),-1,/home/test && \,/home/owt-server/source/agent/webrtc/webrtcLib/binding.gyp && \)
...
RUN echo "[mongodb-org-3.6]" >> /etc/yum.repos.d/mongodb-org-3.6.repo && \
    ...
    ifelse(index(DOCKER_IMAGE,xeon-),-1,yum install intel-gpu-tools mesa-libGL-devel libvdpau -y && \)
