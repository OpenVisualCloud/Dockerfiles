...
RUN cd BUILD_HOME && \
    ...
    (cd BUILD_DESTDIR`'BUILD_PREFIX && mkdir -p lib/pkgconfig && mv ssl/lib/pkgconfig/*.pc lib/pkgconfig) && \
    make install && \
    (cd BUILD_PREFIX && mkdir -p lib/pkgconfig && mv ssl/lib/pkgconfig/*.pc lib/pkgconfig)
')
