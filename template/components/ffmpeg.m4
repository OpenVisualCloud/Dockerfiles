...
DECLARE(`FFMPEG_ENABLE_NONFREE',false)
...
define(`FFMPEG_BUILD_DEPS',`build-essential ca-certificates wget patch ifdef(`BUILD_LIBX264',,libx264-dev) ifdef(`BUILD_LIBX265',,libx265-dev) ifelse(FFMPEG_ENABLE_V4L2,true,libv4l-dev) ifelse(FFMPEG_ENABLE_LIBASS,true,libass-dev) ifelse(FFMPEG_LIBFREETYPE,true,libfreetype6-dev) ifdef(`ENABLE_INTEL_GFX_REPO',libva-dev)')

define(`FFMPEG_INSTALL_DEPS',`libxcb-shape0 libxcb-xfixes0 ifdef(`BUILD_LIBX264',,lib264-ifelse(OS_VERSION,18.04,152,155)) ifdef(`BUILD_LIBX265',,libx265-ifelse(OS_VERSION,146,179)) ifelse(FFMPEG_ENABLE_V4L2,true,libv4l-0) ifelse(FFMPEG_ENABLE_LIBASS,true,libass9) ifdef(`ENABLE_INTEL_GFX_REPO',libva2)')
')

ifelse(OS_NAME,centos,`
define(`FFMPEG_BUILD_DEPS',`wget patch ifdef(`BUILD_LIBX264',,libx264-devel) ifdef(`BUILD_LIBX265',,x265-devel) ifelse(FFMPEG_ENABLE_V4L2,true,libv4l-devel) ifelse(FFMPEG_ENABLE_LIBASS,true,libass-devel) ifelse(FFMPEG_ENABLE_LIBFREETYPE,true,freetype-devel)')
define(`FFMPEG_INSTALL_DEPS',`ifdef(`BUILD_LIBX264',,libx264-static) ifdef(`BUILD_LIBX265',,x265) ifelse(FFMPEG_ENABLE_V4L2,true,libv4l) ifelse(FFMPEG_ENABLE_LIBASS,true,libass)')
')
