...
define(`HIDE',`pushdef(`_n', divnum)divert(-1)')
define(`UNHIDE',`divert(_n)popdef(`_n')')
...
# Same as ``ADD_PKG'' but accepts any number of packages via ``$@'' arguments.
...
ifelse($#,2,$1,`REMOVE($1,shift(shift($@)))'),`^$2$'),`^$2 '),` $2$'),` $2 ',` ')')')
...
`ifelse($#,0,,$#,1,`ifdef(`BUILD_$1',`indir(`BUILD_$1')')',dnl
`ifdef(`BUILD_$1',`indir(`BUILD_$1')')'
`BUILD_COMPONENTS(shift($@))')')
...
pushdef(`_deps',GET_BUILD_DEPS(ARGS(COMPONENTS_LIST)))dnl
pushdef(`_pkgs',PKGS(REMOVE_DUPLICATES(_deps)))dnl
